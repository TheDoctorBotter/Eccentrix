...
import type { OTEvalGoalEntry, OTGoalCategory, OTGoalProgressEntry } from '@/types/notes/ot';
import { OT_GOAL_CATEGORY_LABELS } from '@/types/notes/ot';

// ---------------------------------------------------------------------------
// OTGoalEntryTable — used in OT Evaluation & Re-Evaluation for STG/LTG
// ---------------------------------------------------------------------------

interface OTGoalEntryTableProps {
  label: string;
  goals: OTEvalGoalEntry[];
  onChange: (goals: OTEvalGoalEntry[]) => void;
  readOnly?: boolean;
}

export function OTGoalEntryTable({
  label,
  goals,
  onChange,
  readOnly,
}: OTGoalEntryTableProps) {
  const addGoal = () =>
    onChange([
      ...goals,
      {
  ...
  const update = (idx: number, field: keyof OTEvalGoalEntry, value: string) => {
            ...
            <div>
              ...
              {readOnly ? (
                <p className="text-sm mt-1">
                  {OT_GOAL_CATEGORY_LABELS[goal.category as OTGoalCategory] || goal.category}
                </p>
              ) : (
                <Select
                ...
                >
                  ...
                  <SelectContent>
                    {Object.entries(OT_GOAL_CATEGORY_LABELS).map(([k, label]) => (
...
// ---------------------------------------------------------------------------
// OTGoalProgressTable — used in OT Re-Evaluation & Discharge
// ---------------------------------------------------------------------------

interface OTGoalProgressTableProps {
  label: string;
  entries: OTGoalProgressEntry[];
  onChange: (entries: OTGoalProgressEntry[]) => void;
  readOnly?: boolean;
}

export function OTGoalProgressTable({
  label,
  entries,
  onChange,
  readOnly,
}: OTGoalProgressTableProps) {
  const addEntry = () =>
    onChange([
      ...entries,
  ...
  const update = (
    idx: number,
    field: keyof OTGoalProgressEntry,
    value: string
  ) => {
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-left p-2 font-medium text-slate-600 w-36">
                  Outcome
                </th>
                ...
                <tr key={idx} className="border-b border-slate-100">
                  <td className="p-1">
                    <Input
                      ...
                      onChange={(e) =>
                        update(idx, 'goalText', e.target.value)
                      }
                      className="h-8 text-sm"
                      readOnly={readOnly}
                    />
                  ...
                  <td className="p-1">
                    <Input
                      ...
                      onChange={(e) =>
                        update(idx, 'priorStatus', e.target.value)
                      }
                      className="h-8 text-sm"
                      readOnly={readOnly}
                    />
                  ...
                  <td className="p-1">
                    <Input
                      ...
                      onChange={(e) =>
                        update(idx, 'currentStatus', e.target.value)
                      }
                      className="h-8 text-sm"
                      readOnly={readOnly}
                    />
