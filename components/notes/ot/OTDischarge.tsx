...
import { OTGoalProgressTable } from './shared/OTGoalsSection';
import type {
  OTFormData,
  OTDischargeReason,
  OTGoalProgressEntry,
  OTFunctionalStatusEntry,
} from '@/types/notes/ot';
import { OT_DISCHARGE_REASON_LABELS, createEmptyOTFormData } from '@/types/notes/ot';

interface OTDischargeProps {
  formData: OTFormData;
  onChange: (data: OTFormData) => void;
  readOnly?: boolean;
}

export default function OTDischarge({
  formData,
  onChange,
  readOnly,
}: OTDischargeProps) {
  useEffect(() => {
    if (!formData.meta?.noteType) {
      onChange(createEmptyOTFormData('discharge'));
    }
  ...
  const functionalStatus = (formData.meta.functionalStatus || []) as OTFunctionalStatusEntry[];
  ...
  const removeFunctionalRow = (idx: number) =>
    updateMeta(
      'functionalStatus',
      functionalStatus.filter((_, i) => i !== idx)
    );

  const updateFunctionalRow = (
    idx: number,
    field: keyof OTFunctionalStatusEntry,
    value: string
  ) => {
            ...
            {readOnly ? (
              <p className="text-sm mt-1">
                {formData.plan.dischargeReason
                  ? OT_DISCHARGE_REASON_LABELS[
                      formData.plan.dischargeReason as OTDischargeReason
                    ]
                  : '—'}
              </p>
            ) : (
              ...
              >
                ...
                <SelectContent>
                  {Object.entries(OT_DISCHARGE_REASON_LABELS).map(
                    ([k, label]) => (
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm">
            Initial vs Discharge Functional Status
          </CardTitle>
        </CardHeader>
        <CardContent>
                ...
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-slate-200">
                      ...
                      <th className="text-left p-2 font-medium text-slate-600 w-28">
                        Change
                      </th>
                      ...
                      <tr key={idx} className="border-b border-slate-100">
                        <td className="p-1">
                          <Input
                            value={entry.area}
                            onChange={(e) =>
                              updateFunctionalRow(idx, 'area', e.target.value)
                            }
        ...
        <CardContent className="space-y-3">
          <div>
            ...
            <Textarea
              ...
              placeholder="Home activity program, frequency, and modifications..."
          ...
          <div>
            ...
            <Textarea
              ...
              onChange={(e) =>
                updatePlan('caregiverTrainingSummary', e.target.value)
              }
              placeholder="Summary of caregiver training provided throughout episode of care..."
              className="min-h-[60px] text-sm"
              readOnly={readOnly}
            />
          </div>
          <div>
            ...
            <Textarea
              ...
              onChange={(e) =>
                updateAssessment('clinicalImpression', e.target.value)
              }
              placeholder="Summary of functional gains, remaining needs, and recommendations..."
          ...
          <div>
            ...
            <Textarea
              ...
              onChange={(e) =>
                updatePlan('followUpInstructions', e.target.value)
              }
              placeholder="Follow-up with physician, return-to-OT criteria..."
              className="min-h-[60px] text-sm"
              readOnly={readOnly}
            />
          </div>
